...
void ILI9488::setRefreshDirection(bool normal) {
    if (hwSPI) spi_begin();
    
    writecommand(ILI9488_MADCTL);  // Write to MADCTL (Memory Access Control) register
    
    uint8_t madctl = 0x48;  // Default value for MADCTL (landscape orientation)
    if (!normal) {
        madctl |= 0x20;  // Set the Vertical Refresh Order bit to change the refresh direction
    }
